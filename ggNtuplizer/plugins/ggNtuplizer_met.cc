...
void ggNtuplizer::fillMET(const edm::Event& e, const edm::EventSetup& es, int year) {
    ...
    if (recoil == 1){
        jetVeto30 += 1;  // should be added by 1 for W
    }
...
metcov00=   pfMETHandle->front().getSignificanceMatrix()(0,0);
metcov01=   pfMETHandle->front().getSignificanceMatrix()(0,1);
metcov10=   pfMETHandle->front().getSignificanceMatrix()(1,0);
metcov11=   pfMETHandle->front().getSignificanceMatrix()(1,1);
